 ...
 */
contract ReaperAutoCompoundLiquidv2_Spirit_SPELL is Ownable, Pausable {
    ...
    address public constant fUSDT = address(0x049d68029688eAbF473097a2fC38ef61633A3C7A);
    address public constant SPELL = address(0x468003B688943977e6130F4F68F23aad939a1040);
    address public rewardToken0 = address(0x10b620b2dbAC4Faa7D7FFD71Da486f5D44cd86f9); //LQDR
    address public rewardToken1 = address(0x468003B688943977e6130F4F68F23aad939a1040);
    uint8 public rewardTokens = 2;
    address public lpPair;
    address public lpToken0; //fUSDT
    address public lpToken1; //SPELL
    ...
    address[] public rewardToken1ToWftmRoute = [rewardToken1, fUSDT, wftm];
    address[] public wftmTofUSDTRoute;
    address[] public fUSDTToSPELLRoute;
    ...
    function chargeFees() internal {
        ...
        if (isEmitting[0] == true) {
            ...
            IUniswapRouterETH(tokenRouter[rewardToken0]).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                rewardBal,
                0,
                rewardToken0ToWftmRoute,
                address(this),
                now.add(600)
            );
        }
        ...
        if (isEmitting[1] == true) {
            ...
            IUniswapRouterETH(tokenRouter[rewardToken1]).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                rewardBal,
                0,
                rewardToken1ToWftmRoute,
                address(this),
                now.add(600)
            );
        }
    ...
    function addLiquidity() internal {
        uint256 balance = IERC20(wftm).balanceOf(address(this));
        IUniswapRouterETH(tokenRouter[lpToken0]).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            balance,
            0,
            wftmTofUSDTRoute,
            address(this),
        ...
        uint256 half = IERC20(fUSDT).balanceOf(address(this)).div(2);
        IUniswapRouterETH(tokenRouter[lpToken1]).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            half,
            0,
            fUSDTToSPELLRoute,
            address(this),
        ...
        uint256 lp0Bal = IERC20(fUSDT).balanceOf(address(this));
        uint256 lp1Bal = IERC20(SPELL).balanceOf(address(this));

        IUniswapRouterETH(spiritRouter).addLiquidity(
            lpToken0,
            lpToken1,
            lp0Bal,
            lp1Bal,
    ...
    function giveAllowances() internal {
        ...
        IERC20(rewardToken1).safeApprove(spiritRouter, 0);
        IERC20(rewardToken1).safeApprove(spiritRouter, uint256(-1));
    }
